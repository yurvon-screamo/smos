...
from benchmarks.common.smos_client import SMOSClient
from benchmarks.common.metrics import compute_kendall_tau_b
        ...
        try:
            ...
            for idx, item in enumerate(ds):
                ...
                if isinstance(pq_raw, str):
                    ...
                    conv["probing_questions"] = (
                        pq_raw if isinstance(pq_raw, dict) else {}
                    )
            ...
            logger.info(
                "Downloaded and cached %s: %d conversations", size, len(conversations)
            )
        ...
        if is_plan_format:
            ...
            for session in chat_data:
                ...
                plan_keys = sorted(
                    session.keys(),
                    key=lambda k: (
                        int(k.split("-")[-1]) if k.split("-")[-1].isdigit() else 0
                    ),
                )
...
def batch_to_chunks(
    turns: list[dict], chunk_size: int = CHUNK_SIZE
) -> list[list[dict]]:
        ...
        if isinstance(type_questions, list):
            for q in type_questions:
                if isinstance(q, dict):
                    ...
                    questions.append(
                        {"question_type": q_type, "question_text": q, "rubric": []}
                    )
...
async def ingest_conversation(
    ...
    category = (
        conv_seed.get("category", "unknown")
        if isinstance(conv_seed, dict)
        else "unknown"
    )
    ...
    if debug:
        ...
        debug_path = os.path.join(
            debug_dir, f"beam_{chat_size}_{conv_idx}_ingestion.txt"
        )
        ...
        if not chunks_already_done:
            ...
            debug_file.write(
                f"Batches: {len(batches)}, Chunks: {total_chunks} (size={CHUNK_SIZE})\n"
            )
    ...
    for batch_idx, batch_turns in enumerate(batches):
        ...
        for chunk_idx, messages in enumerate(chunks):
            ...
            if debug_file:
                debug_file.write(
                    f"--- Chunk {chunk_idx} ({len(messages)} messages) ---\n"
                )
                for msg in messages:
...
async def process_question(
    ...
    for c in cutoffs:
        ...
        for nugget in rubric:
            ns = await judge_single_nugget(
                question_text, nugget, generated_answer, judge_llm
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--judge-model", default="gpt-5", help="Model for rubric judging"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Verbose logging + debug files"
    )
    parser.add_argument(
        "--score-debug",
        action="store_true",
    ...
    parser.add_argument(
        "--run-id", default=None, help="Reuse a specific run_id for resume"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--rpm", type=int, default=200, help="Requests per minute for LLM"
    )
    parser.add_argument(
        "--backend",
        default="oss",
    ...
    parser.add_argument(
        "--mem0-api-key", default=None, help="Mem0 API key (cloud mode only)"
    )
...
async def async_main() -> None:
    ...
    # SMOS replaces Mem0: each add/search/finalize shells out to the unified
    # `smos` binary against SMOS_CONFIG (default smos.bench.toml). The Mem0-
    # shaped kwargs are accepted for parity and ignored by SMOSClient.
    mem0 = SMOSClient(
        mode=backend,
        host=args.mem0_host,
    ...
    if args.evaluate_only:
        ...
        if has_cutoffs:
            ...
            print(
                "Results don't have cutoff_results. Run without --evaluate-only first."
            )
        return
        ...
        with shutdown:
            ...
            for size in chat_sizes:
                ...
                for ci in indices:
                    ...
                    if not success:
                        logger.warning(
                            "[%s][%d] Had failures during ingestion", size, ci
                        )
                ...
                for ci in indices:
                    ...
                    if q_type_filter:
                        questions = [
                            q
                            for q in questions
                            if q.get("question_type") in q_type_filter
                        ]
            ...
            already_done = sum(
                ...
                if f"{size}_{ci}_q{qi}_{q.get('question_type', 'unknown')}"
                in existing_ids
            )
