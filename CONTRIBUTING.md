...
If `cargo t` is green, you have a working tree. `cargo tall` (the full surface, including the 643 MB DeBERTa-v3 ONNX download and live `llama-server` calls) is only needed when your change touches the native NLI path or a live-`llama-server` integration surface.
...
| `cargo tall` | Adds the `#[ignore]` tests: native NLI model download + live `llama-server`. Pre-release only. |
...
- **Live `llama-server`** — `#[ignore = "requires live llama-server on localhost:{28081,28082,28181}"]`
...
3. **Needs a live `llama-server` / model download** → `#[ignore]` per test, with a reason.
...
3. **Run the full local gate before pushing:**
   ...
   Run `cargo tall` only if your change touches the native NLI path or a live-`llama-server` surface.
